...
# Define UI for application 
 ui <- fluidPage(
  ...
  # Application title
 # headerPanel("Is an Elective Cath Needed?"),
   ...
   sidebarLayout(
      ...
      mainPanel(
        ...
        tabsetPanel(
          tabPanel("About", 
                   h3("What Is It?"),
                   p("This app is a risk calculator that uses Machine Learning to determine the likelihood that coronary stenosis will actually be found in a candidate patient for elective angiography."),
                   h3("How Is It Used?"),
                   p("Enter patient pre-test attributes (de-identified) at left to calculate a risk prediction. Customize the data SOURCE(s) if desired, view the MODEL to better understand prediction decisions, and observe the dependence of model VALIDITY on data sources."),
                   h3("How Does It Work?"),
                  ...
                  SOURCE(s), and is applied to a new patient (at left), to compute an expected probability of having a positive cardiac catheterization. The source data for the model is configurable."),
                   h3("To Whom Does It Apply?"),
                   p("The source data sets were collected from new cardiology patients, with no prior (known) coronary artery disease. Source data is included from 4 geographic areas.
                  Each source data set contains patients scheduled for an elective cardiac catheterization, and with NO previous non-invasive (i.e. stress) test. The model 
                  predicts the probability of whether a cath candidate will or will not be found to have coronary disease of >50% stenosis on the cath. 
                  Low-probability results may indicate benefit to choosing a plan of non-invasive testing and medical management instead, coupled with continued primary prevention."),
                   ...
                   p("Due to inclusion criteria of the source data, this does not generalize to the general public or lower risk patients who are not thought to be candidates for angiography. Furthermore, it is not applicable to the setting of acute myocardial infarction.")),
                   #tableOutput("values"),
          ...
          tabPanel("Validity", 
                   ...
                   p("It is important to evaluate machine learning models to show their effectiveness at predicting new events. Cross-validation is an internal sampling method for evaluating whether a model is successful making predictions on new data elements.  The statistics are provided below are 95% confidence intervals based on 500 randomly-seeded repeats of k-fold cross validations, where k=10. 
                   ...
                   or a low overall number of individuals in data selected. As such, the models are generally weak for females, applying filtering by symptoms, and using the VA and Switzerland data. Counterintuitively, the absence of pain is very a strong predictor of disease in these data sets. This may indicate a sampling bias. Thus additional consideration of issues related to clinical validity is also required. In many cases, the primary factor for the model's predictive capacity is that the presence of pain appears to be a protective factor."),
...
# Reactive expression to compose a training data containing the values of appropriate gender
...
#Summary for the selected data set. 
...
#Reactive expression to compose of Test Patient object
  test <- reactive({
    # Compose data frame
    data.frame(
      age = input$age, 
...
#Reactive expression to compose of Model object
  rtree <- reactive({
...
#Cross validation
  ...
  kfolds <- reactive ({ 
    ...
    k<-10
    folds <- createFolds(filtered()$cv_disease, k=k, list=TRUE, returnTrain = TRUE)
    for (i in 1:k) {
      xtree <- rpart(
...
# Primary model function: create the prediction and output the answer.
...
# Show the test patient values using an HTML table
...
# Create the plot of the reference data
